...
void describe('RD', () => {
  ...
  void it('research and development project process', async () => {
    ...
    const javaAvailableInJanuary = await resourceAvailableForCapabilityInPeriod(
      r1,
      Capability.skill('JAVA'),
      JANUARY,
    );
    const r2 = ResourceId.newOne();
    const phpAvailableInFebruary = await resourceAvailableForCapabilityInPeriod(
      r2,
      Capability.skill('PHP'),
      FEBRUARY,
    );
    const r3 = ResourceId.newOne();
    const csharpAvailableInMarch = await resourceAvailableForCapabilityInPeriod(
      r3,
  ...
  const resourceAvailableForCapabilityInPeriod = async (
    resource: ResourceId,
    capability: Capability,
    slot: TimeSlot,
  ): Promise<ResourceId> => {
    await availabilityFacade.createResourceSlots(resource, slot);
